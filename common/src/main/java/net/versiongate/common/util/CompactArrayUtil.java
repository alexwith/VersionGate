...
public interface CompactArrayUtil {
    ...
    static long[] createCompactArrayWithPadding(int bitsPerEntry, int entries, Function<Integer, Long> valueGetter) {
        ...
        for (int i = 0; i < entries; i++) {
            final long value = valueGetter.apply(i);
    ...
    static long[] createCompactArray(int bitsPerEntry, int entries, Function<Integer, Long> valueGetter) {
        final long maxEntryValue = (1L << bitsPerEntry) - 1;
    ...
    static void iterateCompactArray(int bitsPerEntry, int entries, long[] data, BiConsumer<Integer, Integer> consumer) {
        final long maxEntryValue = (1L << bitsPerEntry) - 1;
        for (int i = 0; i < entries; i++) {
            final int bitIndex = i * bitsPerEntry;
            final int startIndex = bitIndex / 64;
